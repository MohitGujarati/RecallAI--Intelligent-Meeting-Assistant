package com.mohit.recall_ai.service.recovery
...
class ProcessDeathRecoveryManager @Inject constructor(
    ...
    /**
     * Resets any GENERATING summaries to PENDING and re-enqueues the
     * summary worker. A summary stuck as GENERATING means the LLM stream
     * was in-flight when the process died.
     *
     * NOTE: SummaryWorker class is defined in Chapter 5.
     */
    private suspend fun requeueStuckSummaries(): Int {
        // Find all meetings with a GENERATING summary
        // (We'd need a DAO query for this — using a workaround via MeetingDao)
        val stoppedMeetings = meetingDao.getActiveSession()
        // Simplified: in practice, enumerate all summaries in GENERATING state
        // Full implementation unlocked when SummaryDao.getGeneratingSummaries() is added

        // Uncomment after Chapter 5:
        /*
        val stuckSummaries = summaryDao.getSummariesByStatus(SummaryStatus.GENERATING)
        for (summary in stuckSummaries) {
            summaryDao.resetForRetry(summary.meetingId)
            // re-enqueue SummaryWorker
        }
        return stuckSummaries.size
        */
